    <div class="div-box div-legend">
      Willkommen auf der persönlichen Webseite von<br>
      <h1>Lucía Andrea Illanes Albornoz</h1><br>
      <h2>Systemprogrammiererin | Systems engineer</h2><br>
      <h3 title="Nin me šara u dalla ea">🏳️‍⚧️ 𒊩𒌆 𒈨 𒄭 𒊏 𒌓 𒈦𒄘𒃼 𒌓𒁺 𒀀 🏳️‍⚧️</h3>

      <span class="span-menu span-heading2">
        <a href="/en<?php echo $inc_uri_orig ?>">English</a> |
        <a href="/de<?php echo $inc_uri_orig ?>">German / Deutsch</a> |
        <a href="/es<?php echo $inc_uri_orig ?>">Spanish / Español</a>
      </span>
      <hr>
      <span class="span-menu">
        <a href="/<?php echo $inc_lang ?>/about">Über mich</a> |
        <a href="/assets/Curriculum%20Vitae%20(Deutsch).pdf">Öffentliches Curriculum Vitae (PDF)</a>
        <a href="/assets/CurriculumVitae.tex">(LaTeX)</a><br>
        <a href="/<?php echo $inc_lang ?>/projects">Open Source Projektbeteiligung</a> |
        <a href="https://github.com/lalbornoz">GitHub</a><br>
        <a href="https://github.com/lalbornoz/dotfiles_shared">Dotfiles</a> |
        <a href="https://github.com/lalbornoz/roarie.nvim">Neovim dotfiles</a>
      </span>
      <hr>
      <span class="span-menu">
        <a href="/<?php echo $inc_lang ?>/arabic_poems">Arabische Gedichte</a> |
        <a href="/<?php echo $inc_lang ?>/stories">Unvollendete Träume</a> |
        <a href="/<?php echo $inc_lang ?>/visual_sundries">Visuelle Grabbelkiste</a> |
        <a href="/<?php echo $inc_lang ?>/etc">Et cetera</a>
      </span>
      <hr>
      <span class="span-menu">
        <a href="https://www.linkedin.com/in/lucía-andrea-illanes-albornoz-9ba62a141">LinkedIn</a> |
        <a href="https://www.xing.com/profile/LucíaAndrea_IllanesAlbornoz">Xing</a><br>
        <a href="/<?php echo $inc_lang ?>/impressum">Impressum</a>
      </span>
    </div>

    <!--
      vim:ts=2 sw=2 expandtab fenc=utf-8 foldmethod=marker nowrap tw=0
      -->
